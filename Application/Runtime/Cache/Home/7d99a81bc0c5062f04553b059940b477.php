...
<head>
	...
	<link rel="stylesheet" href="/blog/public/home/css/site.css">
	<!--[if lt IE 9]>
...
<div class="navbar navbar-inverse navbar-fixed-top">
	<div class="container">
		<div class="navbar-header">
			...
			<a class="navbar-brand hidden-sm" href="<?php echo U('/');?>">yxf</a>
		</div>
		<div class="navbar-collapse collapse" role="navigation">
			<ul class="nav navbar-nav">
				<?php $categories = getCategory(1); ?>
				<?php if(is_array($categories)): $i = 0; $__LIST__ = $categories;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$category): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('/index/category',array('id'=>$category['categoryId']));?>"><?php echo ($category["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
			</ul>
		</div>
	</div>
</div>
<section class="container">
	...
	<li><a href="<?php echo U('/index/category',array('id'=>$article['categoryId']));?>"><?php echo ($article["name"]); ?></a></li>
...
<div class="panel panel-default">
	<div class="panel-body">
		...
		<p class="text-muted">
			...
			<a href="<?php echo U('/index/category',array('id'=>$article['categoryId']));?>"><?php echo ($article["name"]); ?></a>
			<i class="glyphicon glyphicon-time"></i> <?php echo (date('m-d',$article["createAt"])); ?>
			...
			<?php if(is_array($comments)): $i = 0; $__LIST__ = $comments;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><div class="media">
					<div class="media-body">
						<h4 class="media-heading"><?php echo ($item["nickname"]); ?>
							<small><?php echo (date('m-d H:i',$item["createAt"])); ?></small>
						</h4>
...
<script>
	$(function() {
		$('#comment-form').on('submit', function(e) {
			...
			$.post('/blog/Home/Index/comment?id=<?php echo ($_GET['id']); ?>', {nickname: nickname, content: content}, function(data) {
			...
			<div class="panel-body">
				<div class="row">
					...
					<?php if(is_array($categories2)): $i = 0; $__LIST__ = $categories2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$category): $mod = ($i % 2 );++$i;?><div class="col-md-6">
							<a href="<?php echo U('/index/category',array('id'=>$category['categoryId']));?>"><?php echo ($category["name"]); ?></a>
