...
<div class="container">
	<h1>文章列表</h1>
	<p>
		<a href="<?php echo U('admin/article/add');?>" class="btn btn-primary">发表文章</a>
	</p>
	<div class="table-responsive">
		<table class="table table-bordered">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><?php echo (date('m-d H:i',$item["createAt"])); ?></td>
					...
					<td>
						<a href="<?php echo U('admin/article/edit?id='.$item['articleId']);?>">编辑</a>
						<a href="<?php echo U('admin/article/delete?id='.$item['articleId']);?>" onclick="return confirm('确定删除吗?')">删除</a>
					</td>
